...
//Integer literals
/*
 *TODO - Implement unsigned literals 
 */
{IntegerNumeral} 	{ return sym(Terminals.INTEGER_LITERAL); }
{IntegerNumeral} [lL]   { return sym(Terminals.LONG_LITERAL,str().substring(0,len()-1)); }
{IntegerNumeral} [sS]	{  return sym(Terminals.SHORT_LITERAL,str().substring(0,len()-1)); }
{IntegerNumeral} [yY]	{  return sym(Terminals.BYTE_LITERAL,str().substring(0,len()-1)); }
...
//Floating-Point Literals
  ...
  [+-]? {Digits} [fF]            { return sym(Terminals.FLOATING_POINT_LITERAL, str().substring(0,len()-1)); }
